...
var conString =
  "pg://postgres:root@127.0.0.1:5432/chicago_divvy_stations_status";
var pg_connection_divvy_trips =
  "pg://postgres:root@127.0.0.1:5432/chicago_divvy_trips";
...
async function find_places_from_yelp(place, where) {
  ...
  let isWhereAllDigit = /^\d{1,5}$/.test(where);
  let body = {};
  if (!isWhereAllDigit) {
    body = {
      size: 1000,
  ...
  for (let i = 0; i < time_slots.length; i++) {
    if (timeRange == time_slots[i]) {
      ...
      if (!isBeginningOfTimeRangeSet) {
        ...
        if (i < targetTime.getHours()) {
          go_back_in_time_var = targetTime.getHours() - i;

          start_datetime_var_2 = new Date(
            start_datetime_var.getTime() -
              go_back_in_time_var * 60 * 60 * 1000 -
              start_datetime_var.getMinutes() * 60 * 1000 -
              start_datetime_var.getSeconds() * 1000 -
              start_datetime_var.getMilliseconds()
          );
        } else {
          go_back_in_time_var = i - targetTime.getHours();
          start_datetime_var_2 = new Date(
            start_datetime_var.getTime() -
              (24 - go_back_in_time_var) * 60 * 60 * 1000 -
              start_datetime_var.getMinutes() * 60 * 1000 -
              start_datetime_var.getSeconds() * 1000 -
              start_datetime_var.getMilliseconds()
          );
        ...
        var go_forward_in_time_var_2 = start_datetime_var_2.getMinutes() + 2 ;
